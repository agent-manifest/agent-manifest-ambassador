...
function scroll() { setTimeout(function() { chat.scrollTop = chat.scrollHeight; }, 30); }
...
function addRow(prefix, html, type, delay) {
  ...
  return new Promise(function(res) {
    setTimeout(function() {
      ...
      d.innerHTML = '<div class="msg-row"><div class="msg-prefix">' + prefix + '</div><div class="msg-content">' + html + '</div></div>';
      chat.appendChild(d); scroll(); res();
    }, delay);
  });
}
...
function addStepBar(label, current) {
  ...
  d.innerHTML = '<span class="line"></span>' + dots + '<span class="step-label">' + label + '</span>' + dots + '<span class="line"></span>';
...
function showTyping() {
  ...
  d.id = 'typing';
  d.innerHTML = '<span class="typing-label">ambassador</span><div class="typing-dot"></div><div class="typing-dot"></div><div class="typing-dot"></div>';
  chat.appendChild(d); scroll();
}

function hideTyping() { var t = document.getElementById('typing'); if (t) t.remove(); }

function lock() { busy = true; inputField.disabled = true; sendBtn.disabled = true; }
function unlock() { busy = false; inputField.disabled = false; sendBtn.disabled = false; inputField.focus(); }
...
function handleSend() {
  ...
  lock();
  showTyping();
  setTimeout(function() { hideTyping(); processStep(step, val); }, 700);
}

function processStep(s, val) {
  switch (s) {
    case 0:
      manifest.identity = val; step = 1;
      addStepBar('step 2 / 5 -- purpose', 1);
      addRow('ambassador', 'Noted. <span class="chip">' + val + '</span> is your agent\'s declared identity.');
    ...
    case 2:
      manifest.capabilities = val.split(',').map(function(c) { return c.trim(); }).filter(Boolean);
      step = 3;
    ...
    case 3:
      manifest.boundaries = val.split(',').map(function(b) { return b.trim(); }).filter(Boolean);
      step = 4;
      ...
      setTimeout(function() {
        ...
        levels.forEach(function(l) {
          btns += '<button class="autonomy-btn" onclick="pickAutonomy(\'' + l + '\')">' + l + '</button>';
        });
        btns += '</div>';
        d.innerHTML = '<div class="msg-row"><div class="msg-prefix">ambassador</div><div class="msg-content">' + btns + '</div></div>';
        chat.appendChild(d); scroll();
        unlock();
      }, 700);
      break;
    ...
    case 4:
      manifest.autonomy_level = val; step = 5;
      generateManifest();
      break;
  }
}
...
function pickAutonomy(level) {
  document.querySelectorAll('.autonomy-btn').forEach(function(b) { b.disabled = true; });
...
function generateManifest() {
  ...
  setTimeout(function() {
    ...
    setTimeout(function() {
      var d = document.createElement('div');
      d.className = 'msg';
      d.innerHTML = '<div style="margin-left:92px">'
        ...
        + '</div>'
        + '<div class="restart"><a onclick="restartSession()">Generate another manifest &#8594;</a></div>'
        + '</div>';
      chat.appendChild(d); scroll();

      document.getElementById('copyBtn').addEventListener('click', function() {
        var fb = document.getElementById('copyOk');
        var doCopy = function() { fb.classList.add('show'); setTimeout(function() { fb.classList.remove('show'); }, 2000); };
...
function init() {
  ...
  addRow('system', 'Protocol: declaration-first &middot; Schema: agent-manifest-spec.org', 'system', 400);
  setTimeout(function() {
